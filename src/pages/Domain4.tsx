...
\`;`,
  ...
  },
  {
    ...
    knowledge: [
      ...
      'Nullable fields to prevent hallucination: when a field might not be present in the input, make it optional AND nullable. Define: {"type": "string", "nullable": true} or use oneOf: [string, null]. This gives Claude an explicit "I don\'t know" option instead of forcing it to fabricate a value. Example: "phone_number" is optional → if not found, Claude returns null instead of making one up.',
      'The "other" + detail string pattern: for classification fields where the predefined categories might not cover everything, add an "other" option with a companion detail field. Example: {"category": {"enum": ["bug", "feature", "question", "other"]}, "category_detail": {"type": "string", "description": "Required when category is \'other\'"}}. This prevents Claude from shoehorning ambiguous items into wrong categories.',
      'Required vs optional fields: only mark a field as required if it MUST always be present. Over-marking fields as required forces Claude to hallucinate values when the input genuinely doesn\'t contain that information. Make questionable fields optional with nullable: true.',
...
for block in response.content:
  ...
  },
  {
    ...
    knowledge: [
      ...
      'Pydantic for schema + semantic validation: use Pydantic models to validate Claude\'s structured output. Pydantic catches both JSON schema violations (type errors, missing fields) AND semantic errors (value constraints, cross-field rules). Define a Pydantic model with validators: field_validator for single-field rules, model_validator for cross-field rules.',
      'Validation-retry loop with Pydantic: (1) Claude outputs JSON via tool_use. (2) Parse with Pydantic model. (3) If ValidationError, extract the specific error messages. (4) Feed errors back to Claude in the retry prompt. (5) Claude sees "Pydantic says: \'total\' must be positive (got -5)" and corrects it.',
      'Two types of validation errors: schema errors (Pydantic catches: wrong type, missing required field, enum violation) and semantic errors (business logic: negative amounts, dates in wrong order, impossible combinations). Both are retryable with specific error feedback.',
